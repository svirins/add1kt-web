    ...
    <div className="flex w-full flex-row flex-nowrap  place-items-center justify-between">
      {isLeftDisabled ? (
        ...
        <Link href={`/blog/p/${Number(currentPage - 1)}`}>
          <a className="group  inline-flex items-center text-base font-medium transition-all  delay-100 hover:text-orange-600 dark:hover:text-emerald-400">
            <span>{`← ${t("previous_page")}`}</span>
          </a>
        </Link>
      )}
      {isRightDisabled ? (
        ...
        <Link href={`/blog/p/${Number(currentPage + 1)}`}>
          <a className="group  inline-flex items-center font-medium  transition-all  delay-100 hover:text-orange-600 dark:hover:text-emerald-400">
            <span>{`${t("next_page")} →`}</span>
          </a>
        </Link>
      )}
